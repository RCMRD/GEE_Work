...
var with_ndvi = filtered.map(L8addNDVI);

//Reduce NDVI collection to mean value of each pixel
var NDVI_mean = with_ndvi.select("NDVI").reduce(ee.Reducer.mean());

//Get mean value from ^^^ for each region in feature collection
var pointMeans = NDVI_mean.reduceRegions({
  collection: wells, 
...
Export.table.toDrive({
  collection: pointMeans,
  description: 'Mean_NDVI_for_Wells',
  fileFormat: 'CSV'
});
