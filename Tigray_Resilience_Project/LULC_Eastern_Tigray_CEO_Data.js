...
var L8_imageCollection = ee.ImageCollection("LANDSAT/LC08/C01/T1_SR"),
    ...
    ceo_data = ee.FeatureCollection("users/firsake/Tigray/tigray_ceo_points"),
    roi = ee.FeatureCollection("users/firsake/Tigray/tigray_east_zone");
...
var years_ = ['2015',/*'2013','2008','2003','1998'*/];
var year_image = [
...
function processImage(imCollection, start, end, lyr, cloud, cls,bands){
    ...
    //training sites from L8 in 2015 therefore sample as for L8 2015 and apply signature
    //to other L8 images
  
    var L8_reference_image_2015 = L8_imageCollection.filterDate(start, end)
                                  .filterBounds(roi)
                                  .filter(ee.Filter.lt("CLOUD_COVER_LAND", cloud))//cloud pixel percentage allowance over land
                                  .map(maskLSR);
    
    var L8_final_reference_2015_ROI = L8_reference_image_2015.median().clip(roi.geometry());
    
    
    var rangeBands = {
                nir: L8_final_reference_2015_ROI.select('B5'),
                red: L8_final_reference_2015_ROI.select('B4'),
                blue: L8_final_reference_2015_ROI.select('B2'),
                G_evi: ee.Number(2.5),
                C1_evi: ee.Number(6),
                ...
                swir1: L8_final_reference_2015_ROI.select('B6')
              };
      ...
      //NDVI
      var ndvi = L8_final_reference_2015_ROI.expression({
      ...
      //EVI
      var evi = L8_final_reference_2015_ROI.expression({
      ...
      //SAVI
      var savi = L8_final_reference_2015_ROI.expression({
      ...
      /*//MSAVI
      var msavi = L8_final_reference_2015_ROI.expression({
      ...
      //NDMI
      var ndmi = L8_final_reference_2015_ROI.expression({
        expression: '(nir - swir1) / (nir + swir1)',
        map: rangeBands
      });
      
      
      L8_final_reference_2015_ROI = L8_final_reference_2015_ROI.addBands(ndvi.clip(roi).rename('ndvi'))
      ...
      //print(L8_final_reference_2015_ROI)
    ...
    var newfc_2015 = ceo_data;
    
    
    var points = L8_final_reference_2015_ROI.sampleRegions({
        collection: newfc_2015, 
        properties: ['CLASS'], 
        scale: 30
    ...
    classified = L8_final_reference_2015_ROI.classify(classify_ftn(cls,training,bands_class));
    Map.addLayer(L8_final_reference_2015_ROI, {bands: bands, min: 0, max: 0.3}, lyr+'_Image');
...
function redraw() {
  ...
  if (cls == 'SELECT'){
      ...
      var start_date_year = '-05-01';
      var end_date_year = '-07-31';
